...
const Feed = ({id}) => {
    ...
    return (
        <Layout>
            <h3 className="text-lg font-medium pt-4">{feed.name}
            </h3>
            ...
            <div className="grid grid-cols-3 gap-4" >
                {
                   feed.bundles.length > 0 ? (
                    feed.bundles.map((item: FeedObject) => ( <OneListItem item={item} type={ItemType.BundleType} key={item.id} useSelected={false} allowEdits={false} />
                    ))
                   ) : (
