...
const Bundle = ({id}) => {
    ...
    return (
        <Layout>
            <h3 className="text-lg font-medium pt-4">{bundle.name}
            </h3>
            ...
            <div className="grid grid-cols-3 gap-4" >
                {
                   bundle.feeds.length > 0 ? (
                    bundle.feeds.map((item: FeedObject) => ( <OneListItem item={item} type={ItemType.FeedType} key={item.id} useSelected={false} allowEdits={false} />
                    ))
                   ) : (
